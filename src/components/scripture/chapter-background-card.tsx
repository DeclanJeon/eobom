  ...
  return (
    <details className="group rounded-2xl border border-border/30 bg-card/40 px-4 py-3">
      ...
      <div className="mt-4 space-y-3">
        {background.overview ? (
          <p className="text-body-sm leading-relaxed text-text-muted">{background.overview}</p>
        ) : null}
        {hasThreeEyes ? (
          <div className="grid gap-3 lg:grid-cols-3">
            {background.historical ? (
              <div className="rounded-xl border border-border/30 px-4 py-3">
                <p className="text-label-sm font-semibold text-primary">역사</p>
                <p className="mt-1 text-body-sm leading-relaxed text-text-muted">{background.historical}</p>
              </div>
            ) : null}
            {background.literary ? (
              <div className="rounded-xl border border-border/30 px-4 py-3">
                <p className="text-label-sm font-semibold text-primary">문학</p>
                <p className="mt-1 text-body-sm leading-relaxed text-text-muted">{background.literary}</p>
              </div>
            ) : null}
            {background.theological ? (
              <div className="rounded-xl border border-border/30 px-4 py-3">
                <p className="text-label-sm font-semibold text-primary">신학</p>
                <p className="mt-1 text-body-sm leading-relaxed text-text-muted">{background.theological}</p>
              </div>
            ) : null}
          </div>
        ) : null}
          ...
          <div className="border-t border-border/20 pt-3">
            ...
            <p className="mt-2 flex flex-wrap gap-2">
              {background.keyVerses.map((kv) => (
                <span key={kv.reference} className="chip-gold text-xs px-2 py-1">{kv.reference}{kv.why ? ` · ${kv.why}` : ""}</span>
              ))}
            </p>
          </div>
        ) : null}
          ...
          <div className="border-t border-border/20 pt-3">
            {background.cautions.length ? (
              <p className="text-label-xs leading-relaxed text-text-muted">
                읽을 때 주의: {background.cautions.join(" · ")}
              </p>
            ) : null}
            {background.sources.length ? (
              <p className="mt-2 text-label-xs text-text-muted">
                출처 · {background.sources.map((s) => `${s.title} ${s.license ?? ""}`).join(" · ")}
              </p>
            ) : null}
          </div>
        ) : null}
