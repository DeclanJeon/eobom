    ...
    <section className="mb-8">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {hasRag && (
            <Link href="/story-mirror/reflect" className="text-label-sm text-leaf">
              거울 연결 {latestRagRunCount ?? ""} →
            </Link>
          )}
          <Link
            href="/story-mirror"
      ...
      {hasRag && !hasMatches && (
        <div className="rounded-2xl border border-border bg-white/95 p-4">
          <p className="line-clamp-3 text-body-md text-primary">{latestRagRunSummary}</p>
        </div>
      )}
    </section>
  );
