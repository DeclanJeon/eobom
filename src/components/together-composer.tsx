...
export function TogetherComposer({
  ...
  if (!communityEnabled) {
    return (
      <div className="mb-6 rounded-2xl border border-border bg-white/95 px-4 py-4">
    ...
    <>
      <div id="together-composer" className="sticky top-[3.5rem] z-20 -mx-1 mb-6 px-1 md:top-24">
        <button
          ...
          className="group flex w-full items-center gap-3 rounded-2xl border border-border bg-white/95 px-4 py-3.5 text-left shadow-[0_12px_40px_-28px_rgba(27,28,26,0.35)] backdrop-blur transition hover:border-accent-gold/40 hover:shadow-[0_16px_40px_-24px_rgba(27,28,26,0.4)]"
        >
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent
          side="bottom"
          className="flex h-[min(92dvh,720px)] max-h-[92dvh] flex-col gap-0 overflow-hidden rounded-t-2xl border-border bg-background p-0 sm:mx-auto sm:max-w-xl sm:rounded-2xl [&>button]:top-3.5"
        >
