...
export function getChapterBackground(params: { code: string; chapter: number; locale?: string }): ChapterBackground | null {
  ...
  let row = d.prepare(`SELECT * FROM chapter_background WHERE code=? AND chapter=? AND locale=? LIMIT 1`).get(code, params.chapter, locale) as Record<string, unknown> | undefined;
